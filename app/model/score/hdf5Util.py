...
class hdf5File:
    ...
    def save_marker(self, timestamp, description):
        with h5py.File(self.filename, "a") as h5_file:
            ...
            new_marker = np.array(
                [(timestamp, description.encode())], dtype=markers.dtype
            )
        ...
        if not os.path.exists(HDF5_FILENAME):
            with h5py.File(HDF5_FILENAME, "w") as h5_file:
                ...
                marker_dtype = np.dtype(
                    [("timestamp", "f8"), ("description", str_dtype)]
                )
                h5_file.create_dataset(
    ...
    def plot_file(filename: str):
        with h5py.File(filename, "r") as h5_file:
            ...
            for idx, (timestamp, score) in enumerate(zip(timestamps, load_score)):
                ...
                if idx > 0 and timestamp > timestamps[idx - 1] + 2:
                    for missing_time in range(timestamps[idx - 1] + 1, timestamp):
                        ...
                        modified_load_score.append(
                            np.nan
                        )  # Set NaN for missing load score
                ...
                if (
                    marker_idx < len(timestamps_marker)
                    and timestamp == timestamps_marker[marker_idx]
                ):
                    y_marker[marker_idx] = score
                    marker_idx += 1

            return (
                np.array(modified_timestamps),
                np.array(modified_load_score),
                timestamps_marker,
                y_marker,
                descriptions,
            )
