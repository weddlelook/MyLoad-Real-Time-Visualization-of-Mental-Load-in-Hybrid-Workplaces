...
class Recorder(QObject):
    ...
    def __init__(self, callback):
        ...
        self.eegWorker = EegWorker(callback, self.error)
        self.thread = QThread()
    ...
    def new_session(self, session_name):
        ...
        self.hdf5_session = hdf5File(session_name, self.callback)
        self.minimum = None
        self.maximum = None
        self.phase_flag = Phase.PAUSED.value
    ...
    def set_phase(self, phase: int, duration: int = None):
        ...
        def phase_complete(phase: int):
            ...
            if self.minimum and self.maximum:
                self.score_calculator = ScoreCalculator(self.minimum, self.maximum)
    ...
    def _monitoring_phase(self, data):
        try:
            ...
            self.callback.message.emit(
                Callback.Level.INFO, f"Calculated score: {data["load_score"]}"
            )
