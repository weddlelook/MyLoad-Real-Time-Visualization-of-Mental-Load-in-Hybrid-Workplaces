...
class EEGMonitoring(QObject):
    ...
    powers = pyqtSignal(
        dict
    )  # contains timestamp, theta, alpha, beta powers, cognitive load that were last calculated
    ...
    def record_asr_baseline(self, time: int):
        ...
        def _finish_baseline_recording(self):
            try:
                ...
                baseline_data_pp = self._preprocess_data(
                    baseline_data, self.sampling_rate
                )
